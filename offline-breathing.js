...
class OfflineBreathing {
    ...
    startCycle() {
        ...
        // Build sequence
        if (inhale > 0) sequence.push({ phase: 'inhale', duration: inhale });
        if (holdIn > 0) sequence.push({ phase: 'holdIn', duration: holdIn });
        if (exhale > 0) sequence.push({ phase: 'exhale', duration: exhale });
        if (holdOut > 0) sequence.push({ phase: 'holdOut', duration: holdOut });
